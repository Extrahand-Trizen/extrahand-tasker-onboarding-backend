...
export class LeadController {
  ...
  static async searchLeads(req: AdminRequest, res: Response): Promise<void> {
    try {
      ...
      const filters: SearchFilters = {
        ...
        pickedByAny: (Array.isArray(pickedByAny) ? pickedByAny : pickedByAny ? [pickedByAny] : undefined) as string[] | undefined,
  ...
  static async getStatusAnalytics(req: AdminRequest, res: Response): Promise<void> {
    try {
      ...
      const fromDate = from ? (parseISTDateOnly(from as string) ?? new Date(from as string)) : (isAllTime ? new Date(0) : new Date(Date.now() - 6 * 24 * 60 * 60 * 1000));
      const toDate = to ? (parseISTDateOnly(to as string, true) ?? new Date(to as string)) : new Date();
  ...
  static async getTeamPerformance(req: AdminRequest, res: Response): Promise<void> {
    try {
      ...
      const currentUserIds = [req.admin.userId, req.admin.uid].filter(
        (id): id is string => typeof id === 'string' && id.trim().length > 0
      );
  ...
  static async exportStatusReport(req: AdminRequest, res: Response): Promise<void> {
    try {
      ...
      const filters: any = {
        ...
        reportCategory: reportCategory as 'touched_leads' | 'interested' | 'callback_scheduled' | 'callback_overdue' | 'onboarded' | 'verified',
