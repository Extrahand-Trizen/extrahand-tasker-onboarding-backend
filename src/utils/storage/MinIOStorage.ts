...
export class MinIOStorage extends BaseStorage {
  ...
  constructor(config: any = {}) {
    ...
    if (!rawEndpoint) {
      logger.error('❌ MINIO_ENDPOINT environment variable is required but not set');
    ...
    try {
      ...
      logger.error('❌ Invalid MINIO_ENDPOINT format', { rawEndpoint, error: (e as Error).message });
    ...
    logger.info('✅ Using MinIO endpoint directly from MINIO_ENDPOINT', {
    ...
    if (serverUrl) {
      try {
        ...
        logger.info('Using MINIO_SERVER_URL for public domain', { publicDomain: this.publicDomain });
      } catch (e) {
    ...
    if (this.endpoint.includes('.apps.') || this.endpoint.includes('.extrahand.in')) {
      logger.info('ℹ️ Using public domain for MinIO endpoint (suitable for local development)', {
    ...
    logger.info('AWS S3 client endpoint configuration', {
      endpoint: this.endpoint,
      endpointHost,
      endpointPort,
  ...
  private async ensureBucketExists(): Promise<boolean> {
    try {
      ...
      logger.error('Failed to ensure bucket exists:', errorDetails);
  ...
  async uploadFile(
    ...
    try {
      ...
      try {
        ...
        logger.warn('⚠️ Bucket check failed, proceeding with upload anyway', {
